...
names_list = [
    m["properties"]["prosjektnavn"] for m in metadata_all_projects["ProjectMetadata"]
]
...
ortofoto_type_list = [
    m["properties"]["ortofototype"] for m in metadata_all_projects["ProjectMetadata"]
]
...
between_03_05 = sum(
    np.logical_and(np.array(resolution_list) > 0.3, np.array(resolution_list) <= 0.5)
)
between_02_03 = sum(
    np.logical_and(np.array(resolution_list) > 0.2, np.array(resolution_list) <= 0.3)
)
between_01_02 = sum(
    np.logical_and(np.array(resolution_list) > 0.1, np.array(resolution_list) <= 0.2)
)
below_01 = sum(np.array(resolution_list) <= 0.1)
print(
    f"we have a total of projects with a higher (worse) resolution that 0.5 m: {above_05} "
    f"(share: {np.round(above_05/len(resolution_list)*100,1)}%"
)
print(
    f"we have a total of projects with a resolution between 0.3 and 0.5: {between_03_05}"
    f" (share: {np.round(between_03_05/len(resolution_list)*100,1)}%"
)
print(
    f"we have a total of projects with a resolution between 0.2 and 0.3: {between_02_03}"
    f" (share: {np.round(between_02_03/len(resolution_list)*100,1)}%"
)
print(
    f"we have a total of projects with a resolution between 0.1 and 0.2: {between_01_02}"
    f" (share: {np.round(between_01_02/len(resolution_list)*100,1)}%"
)
print(
    f"we have a total of projects with a resolution below 0.1: {below_01} (share: "
...
print(
    f"we have a total of projects with a higher (worse) resolution that 0.3 m: {sum(np.array(resolution_list) > 0.3)}"
)
print(
    f"we have a total of projects with a higher (worse) resolution that 0.5 m: {sum(np.array(resolution_list) > 0.5)}."
)
# %% time histogram
...
print(
    f"we have the following attributes: {metadata_all_projects['ProjectMetadata'][0]['properties'].keys()}"
)
# print(f'with the following values for the first project: {metadata_all_projects['ProjectMetadata'][0]['properties'].values()}')
print(
    f"the st_area(shape) is the area of the project, and looks like this: {metadata_all_projects['ProjectMetadata'][0]['properties']['st_area(shape)']}"
)
...
total_coverage_m2 = int(sum(areas))
print(
    f"the total coverage of all projects is {total_coverage_m2} m^2 - {total_coverage_m2/1e6} km^2"
)
print(
    f"compared to the total landsize of Norway, which is 385,207 km^2, this is {np.round(total_coverage_m2/1e6/385207*100)}%"
)
# let's filter by resolution and calculate the total pixels:
total_pixels = 0
relevant_area = 0
for area, resolution in zip(areas, resolution_list):
    if resolution <= 0.2:
        total_pixels += area / 0.2**2
        relevant_area += area
    elif resolution <= 0.5:
        total_pixels += area / 0.5**2
        relevant_area += area
    else:
        pass
print(
    f"the total number of pixels  for the relevant area of {relevant_area}"
    f" ({np.round(relevant_area/total_coverage_m2,2)*100}%) is {int(total_pixels)} "
    f"- {int(total_pixels/1e12)} trillion pixels"
)
# the data needed for this
print(
    f"to store {int(total_pixels/1e12)} trillion pixels, we need {int(total_pixels*8)} bits (8bits per pixel), "
    f" equivalent to {total_pixels*8/2**40} TB "
)
plt.figure()
...
plt.show()
# %% total area by year
# we can also calculate the total area by year
area_by_year = {y: 0 for y in range(1935, 2024)}
for area, year, res in zip(areas, time_list, resolution_list):
    if res <= 0.5:
        area_by_year[year.year] += area
plt.figure()
plt.bar(area_by_year.keys(), area_by_year.values())
plt.xlabel("Year")
plt.ylabel("Total area")
plt.title("Total area by year")
plt.show()

# %% the uniquely covered area for each year with a sufficient resolution
projects_by_year = {y: [] for y in range(1935, 2024)}
for project, year, res in zip(
    metadata_all_projects["ProjectList"], time_list, resolution_list
):
    if res <= 0.5:
...
legend_elements = [
    Patch(
        facecolor=plt.cm.Paired(i / max(types)),
        edgecolor="r",
        label=type_definitions[i],
    )
    for i in set(types)
]
...
plt.savefig(
    root_dir / "data/figures/orthophoto_metadata/resolution_vs_time.png",
    dpi=300,
    bbox_inches="tight",
)
plt.show()
