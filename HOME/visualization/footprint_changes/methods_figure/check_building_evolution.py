...
for i, project in enumerate(project_list):
    ...
    t1_path = (
        polygon_directory + f"/polygons_{project}_resolution{res}_{x_grid}_{y_grid}.fgb"
    )
    ...
    project_coords.append(
        [[[x, y] for x, y in list(polygon.exterior.coords)] for polygon in t1.geometry]
    )

print(project_coords[2])

# %%
fig, ax = plt.subplots()
plot_skewed_footprints(project_coords[2], ax=ax, skew=0.25, flatten=0.7)
plt.axis("off")
ax.set_aspect("equal")
plt.show()
# %%
