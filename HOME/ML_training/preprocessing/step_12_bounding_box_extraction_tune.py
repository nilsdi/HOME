    ...
    for i in range(len(bbox[city])):
        ...
        if bbox[city][i]["training"] == 1:
            if bbox[city][i]["res"] == res:
                ...
                folder = root_dir / f"data/temp/pretune/"

                if not os.path.exists(
                    root_dir / f"data/temp/pretune/images/{filename}.tif"
                ):
                    data, transform = get_labels(gdf_omrade, bbox_coordinates, res)
                    if data.size != 0:
                        save_labels(
                            data, filename, transform, save_folder=folder / "labels"
                        )
                    ...
                    if image.size != 0:
                        save_cut_geotiff(
                            image, filename, transform, save_folder=folder / "images"
                        )
